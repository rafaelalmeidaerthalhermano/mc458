...
int Comparator (int i, int j, void ** decision) {
...
struct Decision * DecisionTree (int size, void (*Sorter) (int [], int, void *, int (*) (int, int, void **))) {
    struct Decision * decision = (struct Decision *) malloc(sizeof(struct Decision));
    decision->j = decision->i = -1;
    decision->F = decision->T = NULL;

    Sorter(malloc(sizeof(int) * size), size, &decision, &Comparator);
    return decision;
}

void PrintDecisionTree (struct Decision * decisionTree, int indent) {
    int i;
